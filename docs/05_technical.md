...
- **Activation codes:** table `activation_codes`, one row per `venta`. For **physical** deliveries no code is created at sale/payment time: `finalizeVentaPayment.ts` leaves the `venta` code-less, and the team attaches a code from the pre-printed stock (the sticker of the box being packed) in App_Operativo → Pedidos → Por preparar, with `expires_at` = `paid_at` + 180 days. Only **digital** deliveries still get an auto-generated code in `finalizeVentaPayment.ts` when a `venta` transitions to `paid` (`src/features/activation/generateActivationCode.ts`), valid 6 months from purchase. This is the unique code printed inside the physical box — format `VIVA-XXXXXXXX`, 8 random chars (32⁸ ≈ 1.1×10¹² combinations; the alphabet excludes visually ambiguous characters), sized to resist brute-force guessing against a rate-limited endpoint.
