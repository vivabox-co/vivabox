        ...
        <div className="relative pl-8">
          ...
          <div className="space-y-9">
            {MILESTONES.map((item, index) => (
              <Reveal key={item.year} duration={400} delay={index * 90}>
                <div className="relative">
                  <span className="absolute -left-8 top-1.5 w-3.5 h-3.5 rounded-full bg-primary ring-4 ring-ink" />
                  <span className="font-condensed font-semibold text-lg text-primary align-baseline">
                    {item.year}
                  </span>{" "}
                  <span className="font-semibold text-white text-lg align-baseline">
                    {item.title}
                  </span>
                  <p className="text-white/70 leading-relaxed mt-1">{item.copy}</p>
                </div>
              </Reveal>
            ))}
            ...
            <Reveal duration={400} delay={MILESTONES.length * 90}>
              <div className="relative">
                <span className="absolute -left-8 top-1.5 w-3.5 h-3.5 rounded-full bg-white/20 ring-4 ring-ink" />
